...
from model_LSTM import MelSpectrogramLSTM
...
for beta in np.linspace(1.0, 8.0, num=15):
	...
	output_dir = Path(f"LSTM_beta_tuning_outputs/beta_{beta}")
	...
	model_local = MelSpectrogramLSTM(num_classes=num_classes, dropout_rate=0.25)
...
Path("lstm_beta_tuning_outputs").mkdir(exist_ok=True)
pd.DataFrame(summary_rows).to_csv("lstm_beta_tuning_outputs/lstm_beta_sweep_summary.csv", index=False)