...
from model_CNN import MelSpectrogramCNN
...
for beta in np.linspace(1.0, 8.0, num=15):
	...
	output_dir = Path(f"CNN_beta_tuning_outputs/beta_{beta}")
	...
	model_local = MelSpectrogramCNN(num_classes=num_classes, dropout_rate=0.25)
...
Path("cnn_beta_tuning_outputs").mkdir(exist_ok=True)
pd.DataFrame(summary_rows).to_csv("cnn_beta_tuning_outputs/cnn_beta_sweep_summary.csv", index=False)