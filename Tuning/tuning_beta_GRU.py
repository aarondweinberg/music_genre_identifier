# Tuning Basic GRU for beta

# Features: mel pcen spectrograms
#			from 15-second clips, each augmented with pitch shift and pink noise
# Architecture: GRU model
# Values of Beta: 1 through 8 in steps of 0.5
# Inputs: Mel PECN spectrograms
...
from model_GRU import GRUGenreClassifier
...
for beta in np.linspace(1.0, 8.0, num=15):

	print(f"\n=== Training on beta={beta}")

	output_dir = Path(f"GRU_beta_tuning_outputs/beta_{beta}")
	...
	model_local = GRUGenreClassifier(num_classes=num_classes, dropout_rate=0.25)
...
Path("gru_beta_tuning_outputs").mkdir(exist_ok=True)
pd.DataFrame(summary_rows).to_csv("gru_beta_tuning_outputs/gru_beta_sweep_summary.csv", index=False)