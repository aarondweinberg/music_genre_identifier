# Training ResNet18 with a variety of features

# Features: mel spectrograms, chroma, chroma_bs, chroma_cq, hpss_mean, hpss_median, and mfcc
...
exclude_dirs = {"features_csv", "resnet_mel_rgb"}

# Loop over subfolders inside train_base (assuming val_base has matching structure)
for dataset_name in os.listdir(train_base):
	if dataset_name in exclude_dirs:
		continue  # Skip excluded directories

	train_data_directory = os.path.join(train_base, dataset_name)
	val_data_directory = os.path.join(val_base, dataset_name)
	...
	output_dir = Path("outputs") / dataset_name	# Change this output directory name as desired
